...
class ToeicAudioProcessor:
    ...
    def inference_splitted_audios(self, splitted_audio):
        ...
        for audio in splitted_audio:
            question_index = audio.replace('.wav', '').replace('chunk','')
            sentences, sent_start_time = self.inference_audio(audio, api_type='google_stt')
    ...
    def inference_audio(self, path, api_type='google_stt'):
        ...
        if api_type == 'google':
            ...
            sentences, sent_start_time = self.google_STT(path=path)

        return sentences, sent_start_time

    def process_over_1_min(self, audio):
        os.makedirs('./tmp', exist_ok=True)

        print('--------------------')
        print('over 1 min chunk detected!!!!')
        print('processing over 1 min question...')
        ...
        output_ranges = []
        for output_range, _ in chunks:
            output_ranges.append([output_range[0] / 1000 , output_range[1] / 1000])

        print('over 1 min splitted')
        print('making tmp folder..')

        # saving chunks from quetion in temporary folder
        for idx, chunk in enumerate(chunks):
            chunk[1].export('./tmp/{}.wav'.format(idx), format='wav')
            print('processing over 1 min qustion... chunk {}'.format(idx))

        splitted_chunks = sorted(os.listdir('./tmp/'))

        text = []
        sent_start_time = []
        # output_ranges[i][1] : sent end time output_ranges[i+1][1] : next sent end time
        print(output_ranges)

        with open(str(output_ranges[1][0]), 'w') as f:
            for output_range in output_ranges:
                f.write(str(output_range[0]))


        output_interval = [output_ranges[i+1][1] - output_ranges[i][1] for i in range(len(output_ranges) - 1)]
        print('output interval: {}'.format(output_interval))
        for idx, chunk_path in enumerate(splitted_chunks):
            chunk_text, _chunk_sent_start_time = self.google_STT(os.path.join('./tmp/', chunk_path))
            # output_ranges[idx][0]: chunk 시작 시간
            _chunk_sent_start_time = [time + output_ranges[idx][0] for time in _chunk_sent_start_time]
            text.extend(chunk_text)
            sent_start_time.extend(_chunk_sent_start_time)
            # else:
            #     text.extend(chunk_text)
            #     sent_start_time.extend(_chunk_sent_start_time)
    ...
    def google_STT(self, path):
        ...
        # return over 1 min if audio duration over 60s
        audio = AudioSegment.from_wav(path)
        if len(audio) >= 60000:
            text, sent_start_time = self.process_over_1_min(audio)
            # text = 'over 1 min.'
...
if __name__ == '__main__':
    ...
    path = '/root/data/TOEIC_Audio/Toeic_3/TEST_2.mp3'
    toeic = ToeicAudioProcessor()
